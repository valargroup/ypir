...
use crate::serialize::{condense_matrix, pack_vec_pm};

use super::convolution::negacyclic_matrix_u32;
use super::{constants::*, noise_analysis::measure_noise_width_squared, util::*};
...
impl<'a> YClient<'a> {
    ...
    fn from_seed(inner: &'a mut Client<'a>, params: &'a Params, _client_seed: Seed) -> Self {
        Self { inner, params }
    }
    ...
    fn generate_full_query_simplepir(&self, target_idx: u64) -> (AlignedMemory64, AlignedMemory64) {
        // setup
...
#[cfg(test)]
mod test {
    use crate::lwe::{LWEClient, LWEParams};
...
mod malformed_response_tests {
    ...
    fn decode_simplepir_truncated_response_panics() {
        ...
        let y_client = YClient::from_seed(&mut client, &params, fixed_seed());
        let _ =
            YPIRClient::decode_response_simplepir_yclient(&params, &y_client, &vec![0u8; len / 2]);
    }
    ...
    fn decode_simplepir_misaligned_length_panics() {
        ...
        let y_client = YClient::from_seed(&mut client, &params, fixed_seed());
        let _ =
            YPIRClient::decode_response_simplepir_yclient(&params, &y_client, &vec![0u8; len + 1]);
    }
    ...
    fn ypirclient_decode_simplepir_empty() {
        let c = YPIRClient::from_db_sz(1 << 14, 2048 * 14);
...
mod sp_decode_pipeline_tests {
    ...
    fn rlwe_encrypt_decrypt_deterministic_with_fixed_rngs() {
        ...
        let decrypt_with = |rng_seed: [u8; 32], pub_seed: [u8; 32]| -> Vec<u64> {
            ...
            let dec = client.decrypt_matrix_reg(&ct).raw();
            (0..params.poly_len)
                .map(|z| rescale(dec.data[z], params.modulus, params.pt_modulus))
                .collect()
        };
        ...
        assert_eq!(
            r1, r2,
            "same RNG seeds must produce identical decrypt results"
        );
        assert_eq!(r1[0], 7);
    }
    ...
    fn modulus_switch_roundtrip_sp_params() {
        ...
        for z in 0..params.poly_len {
            ...
            assert_eq!(
                got, expected,
                "switch/recover round-trip failed at coeff {z}"
            );
        }
    }
    ...
    fn sp_decode_synthetic_single_rlwe() {
        ...
        let dec = decrypt_ct_reg_measured(
            y_client.client(),
            &params,
            &recovered.ntt(),
            params.poly_len,
    ...
    fn sp_decode_full_pipeline_with_byte_conversion() {
        ...
        let y_client = YClient::from_seed(&mut client, &params, seed);
        let decoded =
            YPIRClient::decode_response_simplepir_yclient(&params, &y_client, &response_bytes);
    ...
    fn pack_query_crt_encoding() {
        ...
        let vals: Vec<u64> = (0..16)
            .map(|i| (i as u64 * 12345) % params.modulus)
            .collect();
        ...
        for (i, (&original, &packed_val)) in vals.iter().zip(packed.as_slice().iter()).enumerate() {
            ...
            assert_eq!(
                crt0,
                original % params.moduli[0],
                "CRT component 0 mismatch at index {i}"
            );
            assert_eq!(
                crt1,
                original % params.moduli[1],
    ...
    fn sp_decode_multi_trial_correctness() {
        ...
        for trial in 0..5u8 {
            ...
            assert_eq!(
                decoded[0], expected_val,
                "trial {trial}: first coeff should be {expected_val}, got {}",
                decoded[0]
            );
        }
    }
