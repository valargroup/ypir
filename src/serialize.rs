...
use crate::client::YPIRSimpleQuery;
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CacheError::Truncated { what, needed, got } => {
                write!(
                    f,
                    "cache truncated: {what} needed {needed} bytes, got {got}"
                )
            }
...
pub(crate) mod cache_io {
    ...
    pub(crate) fn load_aligned_memory64_exact<R: Read>(
        ...
        if on_disk != expected_len_u64 {
            return Err(CacheError::Malformed {
                what,
                detail: format!("length {on_disk} u64s != expected {expected_len_u64} u64s"),
            });
        }
    ...
    pub(super) fn dump_poly_matrix_ntt<W: Write>(w: &mut W, m: &PolyMatrixNTT) -> io::Result<()> {
    ...
    pub(super) fn dump_vec_pmntt<W: Write>(w: &mut W, v: &[PolyMatrixNTT]) -> io::Result<()> {
        write_u64_le(w, v.len() as u64)?;
        for m in v {
    ...
    pub(super) fn load_vec_vec_pmntt_exact<'a, R: Read>(
        ...
        if on_disk_outer != expected_outer {
            return Err(CacheError::Malformed {
                what,
                detail: format!("outer length {on_disk_outer} != expected {expected_outer}"),
            });
        }
        let mut out = Vec::with_capacity(expected_outer);
        for _ in 0..expected_outer {
            ...
            if on_disk_inner != expected_inner {
                return Err(CacheError::Malformed {
                    what,
                    detail: format!("inner length {on_disk_inner} != expected {expected_inner}"),
                });
            }
    ...
    pub(super) fn dump_vec_vec_usize<W: Write>(w: &mut W, v: &[Vec<usize>]) -> io::Result<()> {
        write_u64_le(w, v.len() as u64)?;
        for inner in v {
    ...
    pub(super) fn load_precomp_exact<'a, R: Read>(
        ...
        for _ in 0..shape.outer {
            let m = load_poly_matrix_ntt_exact(r, params, shape.m_rows, shape.m_cols, what)?;
            let on_disk_vm = u64_to_usize(read_u64_le(r, what)?, what)?;
            if on_disk_vm != shape.vm_len {
                return Err(CacheError::Malformed {
                    what,
                    detail: format!("vm length {on_disk_vm} != expected {}", shape.vm_len),
                });
            }
...
impl<'a> OfflinePrecomputedValues<'a> {
    ...
    pub fn load_from<R: Read>(r: &mut R, params: &'a Params) -> Result<Self, CacheError> {
        ...
        // No flags are assigned in PAYLOAD_FORMAT_V1. Reject unknown bits so a
        // future format is not silently mis-loaded by older code.
        ...
        let y_constants_0 =
            cache_io::load_vec_pmntt_exact(r, params, params.poly_len_log2, 1, 1, "y_constants.0")?;
        let y_constants_1 =
            cache_io::load_vec_pmntt_exact(r, params, params.poly_len_log2, 1, 1, "y_constants.1")?;
...
mod test {
    ...
    use crate::{
        ...
        params::{params_for_scenario_simplepir, DbRowsCols, PtModulusBits},
    ...
    #[test]
    #[ignore = "long test: reads every row of a full SP-sized item fixture"]
    fn test_pt_iter() {
        let num_items = 1 << 14;
    ...
    fn test_pt_iter_unaligned_rows() {
        ...
        for row in 0..num_rows {
            ...
            for col in max_filled_col..db_cols {
                assert_eq!(
                    pt_vals[col], 0,
                    "row {row} col {col}: expected zero padding"
                );
            }

            let as_u64s: Vec<u64> = pt_vals[..max_filled_col]
                .iter()
                .map(|&v| v as u64)
                .collect();
