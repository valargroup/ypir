...
mod test {
    ...
    fn u64s_contiguous_bytes_14bit_edge_cases() {
        let bits = 14;
        let max = (1u64 << bits) - 1;
        let vals = vec![0, 1, max, max - 1, 1 << 13, (1 << 13) - 1, 0b10101010101010 & max];
        let bytes = u64s_to_contiguous_bytes(&vals, bits);
        let recovered = contiguous_bytes_to_u64s(&bytes, bits);
        assert_eq!(recovered, vals);
    }
    ...
    fn u64s_contiguous_bytes_various_bit_widths() {
        for bits in [1, 2, 7, 8, 13, 14, 15, 16, 28, 32, 64] {
            ...
            for i in 0..n {
                assert_eq!(recovered[i], vals[i], "round-trip failed at index {i} for bits={bits}");
            }
        }
    }
    ...
    fn u64s_contiguous_bytes_single_element() {
        for bits in [1, 7, 14, 28, 64] {
            ...
            let bytes = u64s_to_contiguous_bytes(&vals, bits);
            let recovered = contiguous_bytes_to_u64s(&bytes, bits);
            assert!(recovered.len() >= 1, "must recover at least 1 element for bits={bits}");
            assert_eq!(recovered[0], vals[0], "single-element round-trip failed for bits={bits}");
        }
    }
