...
#[cfg(feature = "explicit_avx512")]
#[inline(always)]
unsafe fn writeback_avx512(params: &Params, c_cell: &mut u64, sum_lo: __m512i, sum_hi: __m512i) {
    let mut vl = [0u64; 8];
    let mut vh = [0u64; 8];
...
mod test {
    ...
    fn test_negacyclic_mul_db_col() {
        ...
        for z in 0..params.poly_len {
            pol_a.data[z] = fastrand::u64(..params.pt_modulus);
            pol_b.data[z] = fastrand::u64(..params.pt_modulus);
        }
    ...
    fn test_implicit_accumulates_into_c() {
        ...
        for j in 0..b_cols {
            let expected = barrett_u64(&params, c_once.as_slice()[j] + c_once.as_slice()[j]);
            assert_eq!(
                c_twice.as_slice()[j],
                expected,
    ...
    mod rayon_tests {
        ...
        fn test_rayon_deterministic() {
            ...
            assert_eq!(
                c1.as_slice(),
                c2.as_slice(),
                "rayon results not deterministic"
            );
        }
